...
def create_mixture_from_audio_files(m1, m2, raw_to_tokens, sample_tokens,
    ...
    torchaudio.save(f'{save_path}/real_drums.wav',  m1_real.cpu().squeeze(-1), sample_rate=sample_rate)
    torchaudio.save(f'{save_path}/real_bass.wav',  m2_real.cpu().squeeze(-1), sample_rate=sample_rate)
    ...
    torchaudio.save(f'{save_path}/latent_drums.wav', m1.cpu().squeeze(-1), sample_rate=sample_rate)
    torchaudio.save(f'{save_path}/latent_bass.wav', m2.cpu().squeeze(-1), sample_rate=sample_rate)
...
if __name__ == '__main__':
    ...
    if shuffle_dataset:
        ...
        set_bass = sorted(list(os.listdir(bass_audio_files_dir)))
        
        #print(len(set_bass))
        n_elem = 0
        for idx,elem in enumerate(sorted(os.listdir(drums_audio_files_dir))):
            if True: #elem in set_bass:
                if elem[-4:] == '.dur':
                    continue
                
                if set_bass[idx][-4:] == '.dur':
                    continue

                path_drums_track = os.path.join(drums_audio_files_dir, str(elem))
                path_bass_track = os.path.join(bass_audio_files_dir, str(set_bass[idx]))
                
                track_1, _ = torchaudio.load(path_drums_track)
                track_2, _ = torchaudio.load(path_bass_track)
                pair += 1

                pair_path = os.path.join(save_path,str(pair))
                #print(track_1.shape[1]//sample_length)
                available_chunks = []
                print(f"loaded tracks {path_drums_track}, {path_bass_track}")
                ...
                for i in available_chunks:
                    ...
                    #m1 = audio_preprocess(m1, hps)
                    #m2 = audio_preprocess(m2, hps)
                    chunk_path = os.path.join(pair_path,str(i))
                    ...
                    # .cpu().squeeze(-1)
                    print(f"res0 {res_0.shape}")

                    # for sample_res in range(res_0.shape[0]):
                    #     torchaudio.save(f'{chunk_path}/res_bass_{sample_res}.wav', res_0[sample_res,:].cpu().squeeze(-1), sample_rate=sample_rate)
                    #     torchaudio.save(f'{chunk_path}/res_drums_{sample_res}.wav', res_1[sample_res,:].cpu().squeeze(-1), sample_rate=sample_rate)
                    ...
                    # rejection_sampling_latent(log_p_0_sum, log_p_1_sum, log_likelihood_sum, bs)
                    sdr0, sdr1, sdr0_sorted, sdr1_sorted,idx0_latent, idx1_latent = evaluate_sdr(m0, m1, res_0, res_1)
                    sdr0_real, sdr1_real, sdr0_real_sorted, sdr1_real_sorted, idx0, idx1 = evaluate_sdr(m0_real, m1_real, res_0, res_1)
                    
                    print(f" best at idx {idx0[-1]} , {idx1[-1]}")
                    print(f" best sdr0 {sdr0_real_sorted} best sdr1 {sdr1_real_sorted}")
                    torchaudio.save(f'{chunk_path}/res_bass.wav', res_0[idx0[-1]].cpu().squeeze(-1), sample_rate=sample_rate)
                    torchaudio.save(f'{chunk_path}/res_drums.wav', res_1[idx1[-1]].cpu().squeeze(-1), sample_rate=sample_rate)
