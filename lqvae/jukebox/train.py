...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, data_processor, hps):
    ...
    hps.save_iters = len(data_processor.train_loader)
    print_once(f"# of iterations to save {hps.save_iters}")
    ...
    for i, x in logger.get_range(data_processor.train_loader):
        ...
        # Save checkpoint
        with t.no_grad():
            if hps.save and (logger.iters % hps.save_iters == 0 or finished_training): ###################### hps.save_iters == 1
                if ema is not None: ema.swap()
                orig_model.eval()
        ...
        # Sample
        #with t.no_grad():
        #    if (logger.iters % hps.sample_iters) in list(range(1, 1 + hps.iters_before_update)) or finished_training:
        #        if hps.sample and hps.prior:
...
def run(hps="teeny", port=29501, **kwargs):
    from jukebox.utils.dist_utils import setup_dist_from_mpi
    rank, local_rank, device = setup_dist_from_mpi(port=port)
    hps = setup_hparams(hps, kwargs)
    hps.ngpus = 1 #dist.get_world_size()
    ...
    wandb.init(
        ...
        # "sample_lenght": 131072,
        # "sample_rate":22050,
        # "architecture": "lqvae",
        # "dataset": "musdbhq_downsampled",
        # "hps": "hps.vqvae.py",
        # "batch_size":8
